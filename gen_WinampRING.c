...
int init() {
	...
	if(RegOpenKeyEx(HKEY_CURRENT_USER,		//open HKEY_CURRENT_USER\Software\WinampRING
	   ...
	   &reg)==ERROR_SUCCESS)
	{									//get all the setting from the registry
		RegQueryValueEx(reg,"Volume",0,0,NULL,&lenVal);
		if (lenVal>0) {
			RegQueryValueEx(reg,"Volume",0,0,(LPTSTR) s,&lenVal);
			sscanf_s(s,"%i",&volume);
		...
		if (lenVal>0) {
			RegQueryValueEx(reg,"ListenOnModem",0,0,(LPTSTR) s,&lenVal);
			sscanf_s(s,"%i",&listenOnModem);
		}

		RegQueryValueEx(reg,"COMPort",0,0,NULL,&lenVal);
		if (lenVal>0) {
			RegQueryValueEx(reg,"COMPort",0,0,(LPTSTR) s,&lenVal);
			sscanf_s(s,"%i",&comPort);
		}

		RegQueryValueEx(reg,"Percent",0,0,NULL,&lenVal);
		if (lenVal>0) {
			RegQueryValueEx(reg,"Percent",0,0,(LPTSTR) s,&lenVal);
			sscanf_s(s,"%i",&percent);
		}

		RegQueryValueEx(reg,"Popup",0,0,NULL,&lenVal);
		if (lenVal>0) {
			RegQueryValueEx(reg,"Popup",0,0,(LPTSTR) s,&lenVal);
			sscanf_s(s,"%i",&popup);
		}

		RegQueryValueEx(reg,"ChWinVol",0,0,NULL,&lenVal);
		if (lenVal>0) {
			RegQueryValueEx(reg,"ChWinVol",0,0,(LPTSTR) s,&lenVal);
			sscanf_s(s,"%i",&chWinVol);
		}

		RegQueryValueEx(reg,"Enabled",0,0,NULL,&lenVal);
		if (lenVal>0) {
			RegQueryValueEx(reg,"Enabled",0,0,(LPTSTR) s,&lenVal);
			sscanf_s(s,"%i",&enabled);
		}

		RegQueryValueEx(reg,"Action",0,0,NULL,&lenVal);
		if (lenVal>0) {
			RegQueryValueEx(reg,"Action",0,0,(LPTSTR) s,&lenVal);
			sscanf_s(s,"%i",&action);
		}
		
		RegQueryValueEx(reg,"CallerID",0,0,NULL,&lenVal);
		if (lenVal>0) {
			RegQueryValueEx(reg,"CallerID",0,0,(LPTSTR) s,&lenVal);
			sscanf_s(s,"%i",&callerId);
		}
...
void quit() {
	...
	if(RegCreateKeyEx(HKEY_CURRENT_USER,		//opens HKEY_CURRENT_USER\Software\WinampRING
	   ...
	   KEY_WRITE, NULL,&reg,&r)==ERROR_SUCCESS)
	{											//write all the values back to the registry
			sprintf_s(s,20,"%i",volume);
			RegSetValueEx(reg,"Volume",0,REG_SZ,(LPBYTE) &s,(DWORD) strlen(s)+1);

			sprintf_s(s,20,"%i",listenOnModem);
			RegSetValueEx(reg,"ListenOnModem",0,REG_SZ,(LPBYTE) &s,(DWORD) strlen(s)+1);

			sprintf_s(s,20,"%i",comPort);
			RegSetValueEx(reg,"COMPort",0,REG_SZ,(LPBYTE) &s,(DWORD) strlen(s)+1);

			sprintf_s(s,20,"%i",percent);
			RegSetValueEx(reg,"Percent",0,REG_SZ,(LPBYTE) &s,(DWORD) strlen(s)+1);

			sprintf_s(s,20,"%i",popup);
			RegSetValueEx(reg,"Popup",0,REG_SZ,(LPBYTE) &s,(DWORD) strlen(s)+1);

			sprintf_s(s,20,"%i",chWinVol);
			RegSetValueEx(reg,"ChWinVol",0,REG_SZ,(LPBYTE) &s,(DWORD) strlen(s)+1);

			sprintf_s(s,20,"%i",enabled);
			RegSetValueEx(reg,"Enabled",0,REG_SZ,(LPBYTE) &s,(DWORD) strlen(s)+1);

			sprintf_s(s,20,"%i",action);
			RegSetValueEx(reg,"Action",0,REG_SZ,(LPBYTE) &s,(DWORD) strlen(s)+1);

			sprintf_s(s,20,"%i",callerId);
			RegSetValueEx(reg,"CallerID",0,REG_SZ,(LPBYTE) &s,(DWORD) strlen(s)+1);
			...
			RegSetValueEx(reg,"SIPPort",0,REG_SZ,(LPBYTE) sipPort,(DWORD) strlen(sipPort)+1);
